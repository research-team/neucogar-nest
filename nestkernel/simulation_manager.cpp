...
nest::SimulationManager::SimulationManager()
        : clock_(Time::tic(0L)), slice_(0L), to_do_(0L), to_do_total_(0L), from_step_(0L),
          to_step_(0L) // consistent with to_do_ == 0
        , t_real_(0L), simulating_(false), simulated_(false), exit_on_user_signal_(false), inconsistent_state_(false),
          print_time_(false), use_wfr_(true), wfr_comm_interval_(1.0), wfr_tol_(0.0001), wfr_max_iterations_(15),
...
nest::SimulationManager::update_() {
    ...
    // measure process time (CPU usage for process)
    unsigned short threads_num = kernel().vp_manager.get_num_threads();
    float *process_time_for_step = new float[to_do_total_ / step_size * threads_num](); // Init be zeros;
    float *memory_usage_for_step = new float[to_do_total_ / step_size](); // Init be zeros

    unsigned int array_index = 0;
    unsigned int iteration = 0;

    struct rusage usage;    // Will be used only by master
    ...
    #pragma omp parallel
    {
        ...
        do {
            // the main loop
            // measure CPU at the begin
            clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &cpu_time);
            cpu_begin = cpu_time.tv_sec + cpu_time.tv_nsec * 1e-9;


            if (print_time_)
                gettimeofday(&t_slice_begin_, NULL);

            // ToDo check this block on OpenMP thread wait (barrier) for CPU time
            if (kernel().sp_manager.is_structural_plasticity_enabled() && (clock_.get_steps() + from_step_)
            ...
            if (kernel().node_manager.wfr_is_used()) {
                ...
                const std::vector<Node *> &thread_local_wfr_nodes = kernel().node_manager.get_wfr_nodes_on_thread(thrd);
            ...
            } // end of if(wfr_is_used)
            // end of preliminary update

            const std::vector<Node *> &thread_local_nodes = kernel().node_manager.get_nodes_on_thread(thrd);
            for (std::vector<Node *>::const_iterator node = thread_local_nodes.begin();
                ...
                try {
                    ...
                    exceptions_raised.at(thrd) = lockPTR<WrappedThreadException>(new WrappedThreadException(e));
                }
            }

            // Get CPU time at the end
            clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &cpu_time);
            cpu_end = cpu_time.tv_sec + cpu_time.tv_nsec * 1e-9;
            // Save data to array
            process_time_for_step[array_index * threads_num + thrd] += (cpu_end - cpu_begin);
            ...
            #pragma omp master
            {
                ...
                if (iteration % step_size == 0) {
                    ...
                    memory_usage_for_step[array_index] = usage.ru_maxrss >> 10;
                    array_index += 1;
                }
            }
        ...
        } while (to_do_ > 0 and not exit_on_user_signal_ and not exceptions_raised.at(thrd));
    ...
    std::cout << "======= CPU/MEM USAGE =======" << std::endl;
    for (unsigned int line = 0; line < array_index; ++line) {
        for (unsigned int thread = 0; thread < threads_num; ++thread)
            std::cout << process_time_for_step[line * threads_num + thread] << "  ";
        std::cout << memory_usage_for_step[line] << "MB" << std::endl;
    }
    std::flush(std::cout);
...
void nest::SimulationManager::finalize_simulation_() {
    if (not simulated_)
        return;

    // Check for synchronicity of global rngs over processes
    // TODO: This seems double up, there is such a test at end of simulate()
    if (kernel().mpi_manager.get_num_processes() > 1) if (!kernel().mpi_manager.grng_synchrony(
            kernel().rng_manager.get_grng()->ulrand(100000))) {
        throw KernelException(
