...
public:
   ...
   */
  void send( Event& e, thread t, double, const STDPH5CommonProperties& cp );
...
private:
  ...
  void
  update_weight_( double c0, double n0, double minus_dt, const STDPH5CommonProperties& cp );

  void process_h5_spikes_( const vector< spikecounter >& h5_spikes,
    double t0,
    double t1,
    const STDPH5CommonProperties& cp );
  void facilitate_( double kplus, const STDPH5CommonProperties& cp );
  void depress_( double kminus, const STDPH5CommonProperties& cp );
...
STDPH5Connection< targetidentifierT >::update_serotonin_(
  ...
  const STDPH5CommonProperties& cp )
{
  double minus_dt = h5_spikes[h5_spikes_idx_].spike_time_ - h5_spikes[h5_spikes_idx_ + 1].spike_time_;
...
template < typename targetidentifierT >
inline void
STDPH5Connection< targetidentifierT >::update_weight_( double c0,
  double n0,
  double minus_dt,
  const STDPH5CommonProperties& cp )
{
  const double tau_ave_ = 1/(1 / cp.tau_c_ + 1 / cp.tau_n_);
...
{
  // h5 trace at t0
  double n0 = n_ * std::exp( ( h5_spikes[h5_spikes_idx_].spike_time_ - t0 ) / cp.tau_n_ );
...
template < typename targetidentifierT >
inline void
STDPH5Connection< targetidentifierT >::facilitate_( double kplus,
  const STDPH5CommonProperties& cp )
{
  c_ += cp.A_plus_ * kplus;
}
...
template < typename targetidentifierT >
inline void
STDPH5Connection< targetidentifierT >::depress_( double kminus,
  const STDPH5CommonProperties& cp )
{
