...
hh_cond_exp_traub_dynamics( double, const double y[], double f[], void* pnode )
{
    ...
    // ionic currents
    const double I_Na = node.P_.g_Na * y[ S::HH_M ] * y[ S::HH_M ] * y[ S::HH_M ] * y[ S::HH_H ] * ( y[ S::V_M ] - node.P_.E_Na );
    const double I_K = node.P_.g_K * y[ S::HH_N ] * y[ S::HH_N ] * y[ S::HH_N ] * y[ S::HH_N ] * ( y[ S::V_M ] - node.P_.E_K );
    ...
    //std::cout << "S_.r_ " << S_.r_ << std::endl;

    // membrane potential
    f[ S::V_M ] = ( -I_Na - I_K - I_L - I_syn_exc - I_syn_inh + node.B_.I_stim_ + node.P_.I_e ) / node.P_.C_m;

    //std::cout << "V_M " << f[ S::V_M ] << std::endl;

    // numbers based on testing and printing f[ S::V_M ] on the screen
    if (f[ S::V_M ] < -200){
    	f[ S::V_M ] = -200;
     }
...
nest::hh_cond_exp_traub::Parameters_::Parameters_()
    ...
    , t_ref_( 3.0 )		// Refractory time in ms
...
nest::hh_cond_exp_traub::update(Time const& origin,
    ...
    for ( long lag = from; lag < to; ++lag ) {
        ...
        if ( S_.r_ ){
            --S_.r_;
            // the "spike_***_.get_value()" calling is necessary for starting some
            // internal calculations. Their results on these steps not needed
            // but methods must be invoked!
            B_.spike_exc_.get_value( lag );
            B_.spike_inh_.get_value( lag );
        }
        else
        {
            // now it depends on t_ref
