...
// target index addressing) derived from generic connection template
    ...
    private:
        ...
        facilitate_(double w, double kplus) {
            float norm_w = (w / Wmax_) + (lambda_ * std::pow(1.0 - (w / Wmax_), mu_plus_) * kplus);
            return norm_w < 1.0 ? norm_w * Wmax_ : Wmax_;
        }

        float
        depress_(double w, double kminus) {
            float norm_w = (w / Wmax_) - (alpha_ * lambda_ * std::pow(w / Wmax_, mu_minus_) * kminus);
 ...
 */
    ...
    STDPConnection<targetidentifierT>::STDPConnection()
            : ConnectionBase(), weight_(1.0), tau_plus_(20.0), lambda_(0.01), alpha_(1.0), mu_plus_(1.0),
              mu_minus_(1.0), Wmax_(100.0), Kplus_(0.0) {
    }

    template<typename targetidentifierT>
    STDPConnection<targetidentifierT>::STDPConnection(
            const STDPConnection<targetidentifierT> &rhs)
            : ConnectionBase(rhs), weight_(rhs.weight_), tau_plus_(rhs.tau_plus_), lambda_(rhs.lambda_),
              alpha_(rhs.alpha_), mu_plus_(rhs.mu_plus_), mu_minus_(rhs.mu_minus_), Wmax_(rhs.Wmax_),
              Kplus_(rhs.Kplus_) {
    }
    ...
    STDPConnection<targetidentifierT>::get_status(DictionaryDatum &d) const {
        ...
        def<float>(d, names::weight, weight_);
        def<float>(d, "tau_plus", tau_plus_);
        def<float>(d, "lambda", lambda_);
        def<float>(d, "alpha", alpha_);
        def<float>(d, "mu_plus", mu_plus_);
        def<float>(d, "mu_minus", mu_minus_);
    ...
    STDPConnection<targetidentifierT>::set_status(const DictionaryDatum &d,
        ...
        updateValue<float>(d, names::weight, weight_);
        updateValue<float>(d, "tau_plus", tau_plus_);
        updateValue<float>(d, "lambda", lambda_);
        updateValue<float>(d, "alpha", alpha_);
        updateValue<float>(d, "mu_plus", mu_plus_);
        updateValue<float>(d, "mu_minus", mu_minus_);
