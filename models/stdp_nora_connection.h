...
#ifndef STDP_NORA_CONNECTION_H
#define STDP_NORA_CONNECTION_H

/* BeginDocumentation

   Name: stdp_dopamine_synapse - Synapse type for dopamine-modulated spike-timing dependent
   plasticity.
 ...
 */
class STDPNoraCommonProperties : public CommonSynapseProperties
{
public:
  /**
   * Default constructor.
...
inline long
STDPNoraCommonProperties::get_vt_gid() const
{
  if ( vt_ != 0 )
    return vt_->get_gid();
  else
...
class STDPNoraConnection : public Connection< targetidentifierT >
{

public:
  typedef STDPNoraCommonProperties CommonPropertiesType;
   ...
   */
  STDPNoraConnection( const STDPNoraConnection& );
   ...
   */
  void send( Event& e, thread t, double, const STDPNoraCommonProperties& cp );

  void trigger_update_weight( thread t,
    const vector< spikecounter >& nora_spikes,
    double t_trig,
    const STDPNoraCommonProperties& cp );
...
private:
  ...
  void update_nora_( const vector< spikecounter >& nora_spikes,
    const STDPNoraCommonProperties& cp );

  void
  update_weight_( double c0, double n0, double minus_dt, const STDPNoraCommonProperties& cp );

  void process_nora_spikes_( const vector< spikecounter >& nora_spikes,
    double t0,
    double t1,
    const STDPNoraCommonProperties& cp );
  void facilitate_( double kplus, const STDPNoraCommonProperties& cp );
  void depress_( double kminus, const STDPNoraCommonProperties& cp );
...
template < typename targetidentifierT >
STDPNoraConnection< targetidentifierT >::STDPNoraConnection()
  : ConnectionBase()
  , weight_( 1.0 )
  , Kplus_( 0.0 )
  , c_( 0.0 )
...
template < typename targetidentifierT >
STDPNoraConnection< targetidentifierT >::STDPNoraConnection( const STDPNoraConnection& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
  , Kplus_( rhs.Kplus_ )
  , c_( rhs.c_ )
  , n_( rhs.n_ )
  , nora_spikes_idx_( rhs.nora_spikes_idx_ )
...
template < typename targetidentifierT >
void
STDPNoraConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
STDPNoraConnection< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
...
template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::update_nora_(
  const vector< spikecounter >& nora_spikes,
  const STDPNoraCommonProperties& cp )
{
	++k_;
  double minus_dt =
    nora_spikes[ nora_spikes_idx_ ].spike_time_ - nora_spikes[ nora_spikes_idx_ + 1 ].spike_time_;
  double average_t;
  if (nora_spikes.size() > 0){
  		average_t =  nora_spikes[ nora_spikes_idx_ ].spike_time_ / k_;
  }
  double dt1_ = nora_spikes[ nora_spikes_idx_ ].spike_time_ - prev_;
  n_ = n_ * std::exp( minus_dt / cp.tau_n_ )
    + (std::exp(dt1_  / average_t) - 1) * nora_spikes[ nora_spikes_idx_ + 1 ].multiplicity_ / cp.tau_n_;
  prev_ = nora_spikes[ nora_spikes_idx_].spike_time_;
  ++nora_spikes_idx_;
}

template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::update_weight_( double c0,
  double n0,
  double minus_dt,
  const STDPNoraCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::process_nora_spikes_(
  const vector< spikecounter >& nora_spikes,
  double t0,
  double t1,
  const STDPNoraCommonProperties& cp )
{
  ...
  if ( ( nora_spikes.size() > nora_spikes_idx_ + 1 )
    && ( nora_spikes[ nora_spikes_idx_ + 1 ].spike_time_ <= t1 ) )
  {	
    ...
    double n0 = n_ * std::exp( ( nora_spikes[ nora_spikes_idx_ ].spike_time_ - t0 )
                         / cp.tau_n_ ); // dopamine trace n at time t0
    update_weight_( c_, n0, t0 - nora_spikes[ nora_spikes_idx_ + 1 ].spike_time_, cp );
    update_nora_( nora_spikes, cp );

    // process remaining dopa spikes in (t0, t1]
    double cd;
    while ( ( nora_spikes.size() > nora_spikes_idx_ + 1 )
      && ( nora_spikes[ nora_spikes_idx_ + 1 ].spike_time_ <= t1 ) )
    {
      // propagate weight up to next dopa spike and update dopamine trace
      // weight and dopamine trace n are at time of last dopa spike td but eligibility c is at time
      // t0
      cd = c_ * std::exp( ( t0 - nora_spikes[ nora_spikes_idx_ ].spike_time_ )
                  / cp.tau_c_ ); // eligibility c at time of td
      update_weight_( cd,
        n_,
        nora_spikes[ nora_spikes_idx_ ].spike_time_
          - nora_spikes[ nora_spikes_idx_ + 1 ].spike_time_,
        cp );
      update_nora_( nora_spikes, cp );
    }
    ...
    update_weight_( cd, n_, nora_spikes[ nora_spikes_idx_ ].spike_time_ - t1, cp );
  }
  else
  {
...
template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::facilitate_( double kplus,
  const STDPNoraCommonProperties& cp )
{
  c_ += cp.A_plus_ * kplus;
}
...
template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::depress_( double kminus,
  const STDPNoraCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::send( Event& e,
  thread t,
  double,
  const STDPNoraCommonProperties& cp )
{
  ...
  const vector< spikecounter >& nora_spikes = cp.vt_->deliver_spikes();
  ...
  while ( start != finish )
  {
    process_nora_spikes_( nora_spikes, t0, start->t_ + dendritic_delay, cp );
  ...
  process_nora_spikes_( nora_spikes, t0, t_spike, cp );
...
template < typename targetidentifierT >
inline void
STDPNoraConnection< targetidentifierT >::trigger_update_weight( thread t,
  const vector< spikecounter >& nora_spikes,
  const double t_trig,
  const STDPNoraCommonProperties& cp )
{
  ...
  get_target( t )->get_history(
    t_last_update_ - dendritic_delay, t_trig - dendritic_delay, &start, &finish );
  ...
  process_nora_spikes_( nora_spikes, t0, t_trig, cp );
  n_ = n_ * std::exp( ( nora_spikes[ nora_spikes_idx_ ].spike_time_ - t_trig ) / cp.tau_n_ );
